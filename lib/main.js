...
export default {
  ...
  activate() {
    ...
    helpers.exec(atom.config.get('linter-ansible.ansibleLintExecutablePath'), ['-T']).then(output => {
  ...
  provideLinter() {
    return {
      ...
      lint: (activeEditor) => {
        ...
        if (atom.config.get('linter-ansible.blacklist') !== '') {
          blacklist = new RegExp(atom.config.get('linter-ansible.blacklist'))
        ...
        if (atom.config.get('linter-ansible.useProjectConfig')) {
          ...
          if (atom.config.get('linter-ansible.ruleSkips') != '')
            args = args.concat(['-x', atom.config.get('linter-ansible.ruleSkips')])

          // add additional rules directories
          if (atom.config.get('linter-ansible.rulesDirs')[0] !== '') {
            if (atom.config.get('linter-ansible.rulesDirDefault'))
              args.push('-R')

            for (i = 0; i < atom.config.get('linter-ansible.rulesDirs').length; i++)
              args = args.concat(['-r', atom.config.get('linter-ansible.rulesDirs')[i]]);
          }

          // exclude certain directories from checks
          if (atom.config.get('linter-ansible.excludeDirs')[0] != '')
            for (i = 0; i < atom.config.get('linter-ansible.excludeDirs').length; i++)
              args = args.concat(['--exclude', atom.config.get('linter-ansible.excludeDirs')[i]]);
        }
        ...
        return helpers.exec(atom.config.get('linter-ansible.ansibleLintExecutablePath'), args, {cwd: require('path').dirname(file), ignoreExitCode: true, timeout: atom.config.get('linter-ansible.timeout') * 1000}).then(output => {
        ...
        .catch(error => {
          ...
          // output other errors directly to Atom notification display
            ...
            atom.notifications.addError(
              'An unexpected error with ansible, ansible-lint, linter-ansible, atom, linter, and/or your playbook, has occurred.',
              {
